...
#if defined(AUX_DEBUG_ON)
#define AUX_DEBUG_ERROR(message) aux::report_debug_error__SHOULD_NOT_BE_USED_DIRECTLY(L ## message)
#define AUX_DEBUG_ASSERT(expression) if (!(expression)) AUX_DEBUG_ERROR("Expression failed: (" #expression ")")
#define AUX_DEBUG_MEMGUARD_BEGIN() aux::begin_debug_memory_guard__SHOULD_NOT_BE_USED_DIRECTLY()
#define AUX_DEBUG_MEMGUARD_END() aux::end_debug_memory_guard__SHOULD_NOT_BE_USED_DIRECTLY()
#else
