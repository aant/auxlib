...
namespace aux
{
	...
	const int32_t max_cursor_dim = 256;
	...
	static HBITMAP create_cursor_color_map(const size2_t& size, const void* data)
	{
		BITMAPINFOHEADER info = {};
		info.biSize = sizeof(BITMAPINFOHEADER);
		info.biBitCount = 32;
		info.biWidth = (LONG)size.w;
		info.biHeight = -(LONG)size.h;
		info.biPlanes = 1;
		...
		i32_t pixel_count = size.w * size.h;
	...
	static HBITMAP create_cursor_mask_map(const size2_t& size)
	{
		i32_t pitch = (size.w + 15) / 16;
		size_t data_size = (size_t)pitch * (size_t)size.h;
		void* temp = alloc_mem(data_size);
		fill_mem(temp, 0xff, data_size);
		HBITMAP bitmap = CreateBitmap(size.w, size.h, 1, 1, temp);
		free_mem(temp);
		return bitmap;
	}

	static HCURSOR create_cursor_object(HBITMAP color_map, HBITMAP mask_map, const point2_t& hot_spot)
	{
		ICONINFO info = {};
		info.fIcon = FALSE;
		info.hbmColor = color_map;
		info.hbmMask = mask_map;
		info.xHotspot = (DWORD)hot_spot.x;
		info.yHotspot = (DWORD)hot_spot.y;
	...
	bool internal__init_input(HWND window, HCURSOR default_cursor)
	{
		...
		input->caps.max_cursor_dim = max_cursor_dim;
		return true;
	}
	...
	void internal__process_raw_input(WPARAM wparam, LPARAM lparam)
	{
		...
		if (GetRawInputData((HRAWINPUT)lparam, RID_INPUT, input->raw_buffer, &size, sizeof(RAWINPUTHEADER)) != (UINT)-1)
		{
	...
	cursor_t* create_cursor(const size2_t& size, const point2_t& hot_spot, const void* data)
	{
		...
		AUX_DEBUG_ASSERT((hot_spot.x >= 0) && (hot_spot.x < size.w));
		AUX_DEBUG_ASSERT((hot_spot.y >= 0) && (hot_spot.y < size.h));
		AUX_DEBUG_ASSERT(data != nullptr);

		if ((size.w > max_cursor_dim) || (size.h > max_cursor_dim))
		{
			return nullptr;
		}

		HBITMAP color_map = create_cursor_color_map(size, data);

		if (color_map != nullptr)
		{
			HBITMAP mask_map = create_cursor_mask_map(size);

			if (mask_map != nullptr)
			{
				HCURSOR handle = create_cursor_object(color_map, mask_map, hot_spot);
